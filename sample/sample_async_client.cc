...
class SampleClient {
 public:
  ...
  std::string Sample(const std::string &request_sample_field) {
    ...
    rpc = _stub->PrepareAsyncSample(&context, request, &queue);
...
class SampleClient {
 public:
  ...
  void Sample(const std::string &request_sample_field) {
    // Prepare request
    SampleRequest request;
    request.set_request_sample_field(request_sample_field);
    ...
    // Create an RPC object
    call->rpc = _stub->PrepareAsyncSample(&call->context, request, &_queue);
...
void RunClient() {
  ...
  for (int i = 0; i < 100; ++i) {
    ...
    client.Sample(request_sample_field);
  }
